...
<div class='editViewContainer container-fluid'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{if $RECORD_ID neq ''}
                ...
                <h3 class="span8" title="{vtranslate('LBL_EDITING', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)} {$RECORD_STRUCTURE_MODEL->getRecordName()}">{vtranslate('LBL_EDITING', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)} - {$RECORD_STRUCTURE_MODEL->getRecordName()}</h3>
            
        {else}
                <h3 class="span8">{vtranslate('LBL_CREATING_NEW', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)}</h4>
		{/if}
                <span class="pull-right">
                    <button class="btn btn-success btn-xs" type="submit"><strong>{vtranslate('LBL_SAVE', $MODULE)}</strong></button>
                    <a class="btn btn-danger btn-xs cancelLink" type="reset" onclick="javascript:window.history.back();">{vtranslate('LBL_CANCEL', $MODULE)}</a>
                </span>
		</div>
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{if $FIELD_MODEL->get('uitype') eq "20" or $FIELD_MODEL->get('uitype') eq "19"}
					{if $COUNTER eq '1'}
						 <div class="span2 mycdivlabel">&nbsp;</div><div class="span10 mycdivfield">&nbsp;</div>
                        </div><div class="row">
						{assign var=COUNTER value=0}
					{/if}
				{/if}
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<div class="span{if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'}10{else}4{/if} mycdivfield" {if $FIELD_MODEL->get('uitype') eq '19'} style='width:85%!important' {assign var=COUNTER value=$COUNTER+1} {/if}>
