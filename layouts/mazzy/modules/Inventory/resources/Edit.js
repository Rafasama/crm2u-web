...
Vtiger_Edit_Js("Inventory_Edit_Js",{
	...
	registerProductAndServicePopup : function() {
		var thisInstance = this;
		var lineItemTable = this.getLineItemContentsContainer();
		lineItemTable.on('click','.lineItemPopup', function(e){
			var element = jQuery(e.currentTarget);
	...
	registerReferenceSelectionEvent : function(container) {
		...
		var sourceFieldElementVal = jQuery('input[name="contact_id"]', container).val();
		if(!this.isEditView() && sourceFieldElementVal!="" && sourceFieldElementVal!="" && sourceFieldElementVal!==undefined){
			this.triggerContactRefEvent(container);
		}
	},
	
	
	triggerContactRefEvent : function(container){
		
		var sourceFieldElement = jQuery('input[name="contact_id"]', container);
		
		sourceFieldElement.trigger(Vtiger_Edit_Js.referenceSelectionEvent, {'source_module' : "Contacts", 'record' : sourceFieldElement.val(), 'selectedName' : sourceFieldElement.attr("data-displayvalue")});
	...
	isEditView: function() {
	    ...
	    if(queryDict['record'] === undefined) return false;
	    else return true;
	},
