...
Vtiger_Edit_Js("Calendar_Edit_Js",{
    ...
    getPopUpParams : function(container) {
		...
		if(sourceFieldElement.attr('name') == 'contact_id') {
			...
			if(parentIdElement.length > 0 && parentIdElement.val().length > 0) {
				var closestContainer = parentIdElement.closest('.mycdivfield');
	...
	registerEventStatusChangeEvent : function(container){
		...
		var followupContainer = this.getForm().find('.followUpContainer');
		//if default value is set to Held then display follow up container
		var defaultStatus = this.getForm().find('select[name="eventstatus"]').val();
		if(defaultStatus == 'Held'){
			followupContainer.show();
		}
		else followupContainer.hide();
		
		this.getForm().find('select[name="eventstatus"]').on('change',function(e){
	...
	registerRecurrenceFieldCheckBox : function() {
		...
		if(thisInstance.getForm().find('input[name="recurringcheck"]').is(':checked')) {
			jQuery('#repeatUI').show();
		} else {
			jQuery('#repeatUI').hide();
		}
    ...
    registerClearReferenceSelectionEvent : function(container) {
        ...
        this.getRelatedContactElement().closest('.mycdivfield').find('.clearReferenceSelection').on('click',function(e){
    ...
    registerEndDateTimeChangeLogger : function(container) {
        container.find('[name="time_end"]').on('changeTime',function(e) {
            ...
            var timeDateElement = timeElement.closest('.mycdivfield').find('[name="due_date"]');
    ...
    registerRelatedContactSpecificEvents : function() {
		...
		form.find('[name="contact_id"]').on(Vtiger_Edit_Js.preReferencePopUpOpenEvent,function(e){
            var form = thisInstance.getForm();
            var parentIdElement  = form.find('[name="parent_id"]');
            var container = parentIdElement.closest('.mycdivfield');
	...
	getReferenceSearchParams : function(element){
		var tdElement = jQuery(element).closest('.mycdivfield');
	...
	registerEvents : function(){
		...
		this.registerEventStatusChangeEvent();
