...
{strip}
    ...
    <div class="modelContainer">
        <div class="modal-header contentsBackground">
            <button class="close" aria-hidden="true" data-dismiss="modal" type="button" title="{vtranslate('LBL_CLOSE')}"><i class="fa fa-close"></i></button>
	...
	<div class="modal-body tabbable" style="padding:0px">
		<ul class="nav nav-tabs" style="margin-bottom:0px;padding-left:5px">
			<li class="active">
		...
		<div class="tab-content overflowVisible">
			...
			<div class="{$MODULE_NAME}QuikcCreateContents_{$RAND_NUMBER} tab-pane {if $MODULE_NAME eq 'Events'} active in {/if}fade">
				...
				<div class="quickCreateContent mycdivcalendar">
					<div style='margin:5px; height:280px;' class="table-responsive">
							<div class="row-fluid massEditTable">
							...
							{foreach key=FIELD_NAME item=FIELD_MODEL from=$RECORD_STRUCTURE name=blockfields}
								...
								<div {if $FIELD_MODEL->get('uitype') eq '19' || $FIELD_MODEL->get('name') eq 'description'} class="span9 mycdivfield" {assign var=COUNTER value=$COUNTER+1} {else} class="span4 mycdivfield"{/if}>
				...
				{if $MODULE_NAME eq 'Calendar'}</div>{/if}
				<div class="modal-footer quickCreateActions">
					...
					{/if}
						<a class="btn btn-danger cancelLink cancelLinkContainer pull-right" type="reset" data-dismiss="modal">{vtranslate('LBL_CANCEL', $MODULE)}</a>
