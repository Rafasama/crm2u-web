...
<div id = 'sync_message' class='row-fluid' >
    <div class='padding10 span12'>     
        ...
        {if $STATE eq 'home'}
            {if $SYNCTIME}
            <p id='synctime'><span title="{Vtiger_Util_Helper::formatDateTimeIntoDayString($SYNCTIME)}">{vtranslate('LBL_SYNCRONIZED',$MODULE_NAME)} : {Vtiger_Util_Helper::formatDateDiffInStrings($SYNCTIME)}</span></p>
            {else}
                <p id='synctime'><span>{vtranslate('LBL_NOT_SYNCRONIZED',$MODULE_NAME)}</span></p>    
            {/if}   
       {/if}
        <div class='row-fluid'>
            ...
            <span class="span0">
                <i class="icon-question-sign pushDown" id="popid"  data-placement="left" rel="popover" ></i>
            </span>
        </div>
        <br />
        <div class='row-fluid {if !$FIRSTTIME}hide {/if}' id="removeSyncBlock">
            ...
            <span class="span0">
                <i class="icon-question-sign pushDown" id="removePop"  data-placement="left" rel="popover" ></i>
            </span>
        </div>
    </div>
