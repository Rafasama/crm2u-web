							...
							{foreach item=IMAGE_INFO from=$IMAGE_DETAILS}
								<div class='span2'>
									{if !empty($IMAGE_INFO.path) && !empty({$IMAGE_INFO.orgname})}
										<img class="mazzy-profile" src="{$IMAGE_INFO.path}_{$IMAGE_INFO.orgname}">
									{/if}
								</div>
							{/foreach}
							{if $IMAGE_DETAILS[0]['id'] eq null}
								<div class='span2'>
									<img class="mazzy-profile" src="layouts/mazzy/skins/images/DefaultUserIcon.png">
								</div>
							{/if}
						</div>
						<div class='span6'>
					...
					<div class="pull-right actions">
						<span class="actionImages">
							{if $IS_MODULE_EDITABLE && $LISTVIEW_ENTRY->get('status') eq 'Active'}
								<a class="btn btn-xs btn-yellow"  id="{$MODULE}_LISTVIEW_ROW_{$LISTVIEW_ENTRY->getId()}_EDIT" href='{$LISTVIEW_ENTRY->getEditViewUrl()}'><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="fa fa-pencil alignMiddle"></i></a>&nbsp;
							{/if}
							{if $IS_MODULE_DELETABLE && $LISTVIEW_ENTRY->getId() != $USER_MODEL->getId()}
                                                            {if $LISTVIEW_ENTRY->get('status') eq 'Active'}
                                                                <a id="{$MODULE}_LISTVIEW_ROW_{$LISTVIEW_ENTRY->getId()}_DELETE" class="btn btn-xs btn-red deleteRecordButton"><i title="{vtranslate('LBL_DELETE', $MODULE)}" class="fa fa-trash alignMiddle"></i></a>
                                                            {else}
                                                                <a class="btn btn-xs btn-peas" onclick="Settings_Users_List_Js.restoreUser({$LISTVIEW_ENTRY->getId()}, event);"><i title="{vtranslate('LBL_RESTORE', $MODULE)}" class="fa fa-refresh alignMiddle"></i></a>&nbsp;
                                                                <a class="btn btn-xs btn-red" onclick="Settings_Users_List_Js.deleteUserPermanently({$LISTVIEW_ENTRY->getId()}, event);"><i title="{vtranslate('LBL_DELETE', $MODULE)}" class="fa fa-trash alignMiddle"></i></a>
                                                            {/if}
							{/if}
						</span>
					</div>
