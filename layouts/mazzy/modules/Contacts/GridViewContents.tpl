...
{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
    ...
    border-left: 45px solid transparent;"></div>
			...
			<div class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap>
				...
				{else}
                    ...
                    {else if $LISTVIEW_HEADER->get('uitype') eq '9'}
                    	{$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)|string_format:"%.2f"}
                    {else}
                        {$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}
                    {/if}
				{/if}
			...
			<div class="btn-group alignTop margin0px">
				<span class="pull-right">
					<span class="btn-groups">
						...
						{if $PAGING_MODEL->isPrevPageExists()}<a class="btn listViewPreviousPageButton"  ><span class="icon-chevron-left"></span></a>{/if}
							
							{if $PAGING_MODEL->isNextPageExists() and ($PAGE_COUNT neq 1)}
						<a class="btn listViewNextPageButton"   ><span class="icon-chevron-right"></span></a>{/if}
					</span>
				</span>	
			</div>
        {/if}
