...
Inventory_Edit_Js("Quotes_Edit_Js",{},{
	...
	registerReferenceSelectionEvent : function(container) {
		...
		if(jQuery('input[name="account_id"]').val()!=0 && jQuery('input[name="account_id"]').val()!="" && jQuery('input[name="account_id"]').val()!=undefined){
			var refData = {"source_module": "Accounts", "record": jQuery('input[name="account_id"]').val(), "selectedName":jQuery("#account_id_display").val()};
			thisInstance.referenceSelectionEventHandler(refData, container);
		}
		...
		if(sourceFieldElement.attr('name') == 'contact_id' || sourceFieldElement.attr('name') == 'potential_id') {
			...
			if(parentIdElement.length > 0 && parentIdElement.val().length > 0 && parentIdElement.val() != 0) {
				...
				if(parentIdElement.length > 0 && parentIdElement.val().length > 0) {
					closestContainer = parentIdElement.closest('.mycdivfield');
					params['related_parent_id'] = parentIdElement.val();
					params['related_parent_module'] = closestContainer.find('[name="popupReferenceModule"]').val();
				}
			}
