        ...
        <div class="relatedHeader ">
            <div class="btn-toolbar row-fluid">
                <div class="span6">
                    ...
                    {/if}
            {if $IS_SELECT_BUTTON neq true}name="addButton"{/if}>{if $IS_SELECT_BUTTON eq false}<i class="fa fa-plus"></i>{/if}&nbsp;<strong>{$RELATED_LINK->getLabel()}</strong></button>
    </div>
{/foreach}
&nbsp;
</div>
        ...
        <table class="table table-bordered listViewEntriesTable">
            <thead>
                <tr class="listViewHeaders">
                    <th>{vtranslate('LBL_ACTIONS', $MODULE)}</th>
                    ...
                    {/if}>
                      ...
                      <td nowrap class="{$WIDTHTYPE} ">
                                    ...
                                    <span class="actionImages">
                                        {if $RELATED_MODULE_NAME eq 'Calendar'}
                                            {if $IS_EDITABLE && $RELATED_RECORD->get('taskstatus') neq 'Held' && $RELATED_RECORD->get('taskstatus') neq 'Completed'}
                                                <a class="btn btn-darkyellow btn-xs markAsHeld"><i title="{vtranslate('LBL_MARK_AS_HELD', $MODULE)}" class="fa fa-check alignMiddle"></i></a>
                                            {/if}
                                            {if $IS_EDITABLE && $RELATED_RECORD->get('taskstatus') eq 'Held'}
                                                <a class="btn btn-orange btn-xsholdFollowupOn"><i title="{vtranslate('LBL_HOLD_FOLLOWUP_ON', "Events")}" class="fa fa-flag alignMiddle"></i></a>
                                            {/if}
                                            {if $DETAILVIEWPERMITTED eq 'yes'}
                                                <a class="btn btn-black btn-xs" href="{$RELATED_RECORD->getFullDetailViewUrl()}"><i title="{vtranslate('LBL_SHOW_COMPLETE_DETAILS', $MODULE)}" class="fa fa-th-list alignMiddle"></i></a>
                                            {/if}
                                        {else}
                                            <a class="btn btn-black btn-xs" href="{$RELATED_RECORD->getFullDetailViewUrl()}"><i title="{vtranslate('LBL_SHOW_COMPLETE_DETAILS', $MODULE)}" class="fa fa-th-list alignMiddle"></i></a>
                                        {/if}
                                        {if $IS_EDITABLE}
                                            {if $RELATED_MODULE_NAME eq 'PriceBooks'}
                                                <a class="btn btn-yellow btn-xs" data-url="index.php?module=PriceBooks&view=ListPriceUpdate&record={$PARENT_RECORD->getId()}&relid={$RELATED_RECORD->getId()}&currentPrice={$LISTPRICE}"
                                                   class="editListPrice cursorPointer" data-related-recordid='{$RELATED_RECORD->getId()}' data-list-price={$LISTPRICE}>
                                                    <i class="fa fa-pencil alignMiddle" title="{vtranslate('LBL_EDIT', $MODULE)}"></i>
                                                </a>
                                            {elseif $RELATED_MODULE_NAME eq 'Calendar'}
                                                {if isPermitted($RELATED_MODULE->get('name'), 'EditView', $RELATED_RECORD->getId()) eq 'yes'}
                                                    <a class="btn btn-yellow btn-xs" href='{$RELATED_RECORD->getEditViewUrl()}'><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="fa fa-pencil alignMiddle"></i></a>
                                                {/if}
                                            {else}
                                                <a class="btn btn-yellow btn-xs" href='{$RELATED_RECORD->getEditViewUrl()}'><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="fa fa-pencil alignMiddle"></i></a>
                                            {/if}
                                        ...
                                        {if $IS_DELETABLE}
                                            {if $RELATED_MODULE_NAME eq 'Calendar'}
                                                {if isPermitted($RELATED_MODULE->get('name'), 'Delete', $RELATED_RECORD->getId()) eq 'yes'}
                                                    <a class="btn btn-red btn-xs relationDelete"><i title="{vtranslate('LBL_DELETE', $MODULE)}" class="fa fa-trash alignMiddle"></i></a>
                                                {/if}
                                            {else}
                                                <a class="btn btn-red btn-xs relationDelete"><i title="{vtranslate('LBL_DELETE', $MODULE)}" class="fa fa-trash alignMiddle"></i></a>
                                            {/if}
                        ...
                        {assign var=RELATED_HEADERNAME value=$HEADER_FIELD->get('name')}
                              <td class="{$WIDTHTYPE} rel-field" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>
                            ...
                            {elseif $RELATED_HEADERNAME eq 'listprice' || $RELATED_HEADERNAME eq 'unit_price'}
                                ...
                                {/if}
                            {elseif $RELATED_HEADERNAME eq 'description'}
                            	{$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)|truncate:100:' ...'}
                            {else}
