				...
				{foreach item=RECENT_ACTIVITY from=$RECENT_ACTIVITIES}
					...
					{if $PROCEED}
						{if $RECENT_ACTIVITY->isCreate()}
							<li>
								<div>
									<span><strong>{$RECENT_ACTIVITY->getModifiedBy()->getName()}</strong> <span class="update-success">{vtranslate('LBL_CREATED', $MODULE_NAME)}</span></span>
						...
						{else if $RECENT_ACTIVITY->isUpdate()}
							<li>
								<div>
									<span><strong>{$RECENT_ACTIVITY->getModifiedBy()->getDisplayName()}</strong> <span class="update-info">{vtranslate('LBL_UPDATED', $MODULE_NAME)}</span></span>
