	...
	<div class="listViewPageDiv">
		<div class="listViewTopMenuDiv noprint">
				...
				<span class="btn-toolbar span3">
					<span class="btn-group listViewMassActions">
						{if count($LISTVIEW_MASSACTIONS) gt 0 || $LISTVIEW_LINKS['LISTVIEW']|@count gt 0}
							<button class="btn dropdown-toggle" data-toggle="dropdown"><i class="fa fa-th"></i>&nbsp;<strong>{vtranslate('LBL_ACTIONS', $MODULE)}</strong>&nbsp;&nbsp;<i class="fa fa-caret-down"></i></button>
					...
					{foreach item=LISTVIEW_BASICACTION from=$LISTVIEW_LINKS['LISTVIEWBASIC']}
						<span class="btn-group">
							<button data-toggle="tooltip" data-placement="top" title="{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}" id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_BASICACTION->getLabel())}" class="btn addButton" {if stripos($LISTVIEW_BASICACTION->getUrl(), 'javascript:')===0} onclick='{$LISTVIEW_BASICACTION->getUrl()|substr:strlen("javascript:")};'{else} onclick='window.location.href="{$LISTVIEW_BASICACTION->getUrl()}"'{/if}><i class="fa fa-plus"></i></button>
						</span>
					{/foreach}
					
					{if $smarty.get.view eq "Grid" }
					<span class="btn-group">

							<button data-toggle="tooltip" data-placement="top" title="View List"  class="btn btn-mazzy"  onclick='window.location.href="index.php?module={$MODULE}&view=List"'><i class="fa fa-th-list"></i>{vtranslate('LBL_RECORDS_LIST', $MODULE )}</button>
						</span>
					{else}
					<span class="btn-group">
							<button data-toggle="tooltip" data-placement="top" title="View Grid"  class="btn btn-mazzy"  onclick='window.location.href="index.php?module={$MODULE}&view=Grid"'><i class="fa fa-th-large"></i> {vtranslate('View Grid', $MODULE )}</button>
						</span>
					{/if}
				</span>
			<span class="btn-toolbar span5">
				<span class="customFilterMainSpan btn-group">
					{if $CUSTOM_VIEWS|@count gt 0}
						...
						<select id="customFilterBk" style="">
							{foreach key=GROUP_LABEL item=GROUP_CUSTOM_VIEWS from=$CUSTOM_VIEWS}
							<optgroup label=' {if $GROUP_LABEL eq 'Mine'} &nbsp; {else if} {vtranslate($GROUP_LABEL)} {/if}' >
									{foreach item="CUSTOM_VIEW" from=$GROUP_CUSTOM_VIEWS}
			...
			<span class="hide filterActionImages pull-right">
				...
				<i title="{vtranslate('LBL_DUPLICATE', $MODULE)}" data-value="duplicate" class="icon-plus-sign alignMiddle duplicateFilter filterActionImage pull-right"></i>
			</span>
