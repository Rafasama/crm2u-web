...
jQuery.Class("Vtiger_Detail_Js",{
	...
	selectModuleTab : function(){
		...
		var moduleTab = relatedTabContainer.find('a');
	...
	getSelectedTab : function() {
		...
		return tabContainer.find('a.active');
	},

	getTabContainer : function(){
		return jQuery('.related');
	},

	getTabs : function() {
		return this.getTabContainer().find('li, a');
	},
	...
	registerBlockAnimationEvent : function(){
		...
		detailContentsHolder.on('click','.blockToggle',function(e){
			...
			var closestBlock = currentTarget.closest('.mycdetailblock');
			var bodyContents = closestBlock.find('.mycdetailblockcontent');
	...
	registerBlockStatusCheckOnLoad : function(){
		...
		var blocks = this.getContentHolder().find('.mycdetailblock');
		var module = app.getModuleName();
		blocks.each(function(index,block){
			var currentBlock = jQuery(block);
			var headerAnimationElement = currentBlock.find('.mycdivtitle .blockToggle').not('.hide');
			var bodyContents = currentBlock.find('.mycdetailblockcontent');//.find('tbody')
	...
	registerEventForAddingRelatedRecord : function(){
		...
		detailContentsHolder.on('click','[name="addButton"]',function(e){
			...
			var relatedController = new Vtiger_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModuleName);
			relatedController.addRelatedRecord(element).then(function(response){
						selectedTabElement.trigger( "click" );
			});
		})
	},
	...
	ajaxEditHandling : function(currentTdElement) {
			...
			var saveHandler = function(e) {
				...
				if(currentTdElement.hasClass("mycdivfield")){
					if((element.closest('.mycdivfield').is(currentTdElement))){
						return;
					}	
				}
				else{
					if((element.closest('.row-fluid').is(currentTdElement))){
						return;
					}
				}
	...
	registerSummaryViewContainerEvents : function(summaryViewContainer) {
		...
		summaryViewContainer.on('click', '.summaryViewEdit', function(e){
			var currentTarget = jQuery(e.currentTarget);
			currentTarget.hide();
			var currentTdElement = currentTarget.parent();
	...
	registerEventForRelatedTabClick : function(){
		...
		jQuery('.related').on('click', 'li, a', function(e, urlAttributes){
	...
	registerEvents : function(){
		...
		detailContentsHolder.on('click','.mycdivfield', function(e) {
