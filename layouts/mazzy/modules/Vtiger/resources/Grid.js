...
jQuery.Class("Vtiger_List_Js",{
   ...
   getInstance: function(){
	...
	registerPageNavigationEvents : function(){
		var aDeferred = jQuery.Deferred();
		var thisInstance = this;
		jQuery('#listViewNextPageButton, .listViewNextPageButton').on('click',function(){
			var pageLimit = jQuery('#pageLimit').val();
		...
		jQuery('#listViewPreviousPageButton, .listViewPreviousPageButton').on('click',function(){
			var aDeferred = jQuery.Deferred();
			var pageNumber = jQuery('#pageNumber').val();
			if(pageNumber > 1){
				var orderBy = jQuery('#orderBy').val();
	...
	updatePagination : function(){
		...
		var previousPageButton = jQuery('#listViewPreviousPageButton, .listViewPreviousPageButton');
		var nextPageButton = jQuery('#listViewNextPageButton, .listViewNextPageButton');
	...
	registerChangeCustomFilterEventMobile : function(){
		var thisInstance = this;
		var filterSelectElement = $("#customFilterBk");
		$("#customFilterBk").change(function(e){
			jQuery('#pageNumber').val("1");
			jQuery('#pageToJump').val('1');
			jQuery('#orderBy').val('');
			jQuery("#sortOrder").val('');
			var cvId = $('#customFilterBk').find('option:selected').data('id');
			selectedIds = new Array();
			excludedIds = new Array();
	...
	registerMainCheckBoxClickEvent : function(){
		...
		var thisInstance = this;
		listViewPageDiv.on('click','#listViewEntriesMainCheckBoxTrigger',function(){
			jQuery('#listViewEntriesMainCheckBox').trigger("click");
			var selectedIds = thisInstance.readSelectedIds();
			var excludedIds = thisInstance.readExcludedIds();
			if(jQuery('#listViewEntriesMainCheckBox').is(":checked")){
				jQuery('#listViewEntriesMainCheckBoxTrigger').find("i").attr("class","fa fa-check-square-o");
				...
				jQuery('#listViewEntriesMainCheckBoxTrigger').find("i").attr("class","fa fa-square-o");
				jQuery("#selectAllMsgDiv").hide();
				jQuery('.listViewEntriesCheckBox').each( function(index,element) {
	...
	*/
	...
	registerDuplicateFilterClickEvent : function(){
		...
		var listViewFilterBlock = this.getFilterBlock();
		if(listViewFilterBlock != false){
			listViewFilterBlock.on('mouseup','li i.duplicateFilter',function(event){
				//to close the dropdown
				thisInstance.getFilterSelectElement().data('select2').close();
				var liElement = jQuery(event.currentTarget).closest('.select2-result-selectable');
				var currentOptionElement = thisInstance.getSelectOptionFromChosenOption(liElement);
	...
	registerRowClickEvent: function(){
		...
		listViewContentDiv.on('click','.gridViewEntries',function(e){
	...
	registerEvents : function(){
		...
		this.registerChangeCustomFilterEventMobile();
        ...
        this.registerMYCQuickSearchFieldChange();
        
        this.MYCupdateNavCount();
	},
    ...
    registerListSearch : function() {
      ...
      listViewPageDiv.on('click','[data-trigger="listSearch"]',function(e){
			thisInstance.getListViewRecords({'page': '1'}).then(
					...
					function(data){
						thisInstance.registerMYCQuickSearchFieldChange();
                        //To unmark the all the selected ids
                        jQuery('#deSelectAllMsg').trigger('click');
	...
	registerMYCQuickSearchFieldChange : function(){
		
		var sParams = this.getListSearchParams();
		console.log(sParams);
		if(sParams.length){
			jQuery(".mycQsFieldLabel").hide();
			for(var sc=0; sc<sParams[0].length; sc++){
				
					jQuery("#mycQsField_"+sParams[0][sc][0]+" .sFieldVal").html(sParams[0][sc][2]);
					jQuery("#mycQsField_"+sParams[0][sc][0]).show();
				
				
			}
			jQuery(".mycQuickSearchFieldsTags").show();
		}
		else jQuery(".mycQuickSearchFieldsTags").hide();
		
		jQuery("#mycQuickSearchField").on("change",function(){
			jQuery(".mycQuickSearch").hide();
			jQuery("#mycQuickSearch_"+jQuery(this).val()).show();
		});
	},
	
	MYCupdateNavCount : function(){
			$(".gridrelated").on("click",function(e){
			 
			 
        	 
        	 relElm = $(this).next().find('li a');
        	 
        	 
	             relElm.each(function(n,item){
	             	if($(item).find('.count').length===undefined || $(item).find('.count').length==0){
	             	
		                var url=$(item).attr("data-url");
		                var record=$(item).attr("data-recordid");
