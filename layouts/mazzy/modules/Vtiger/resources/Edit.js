...
jQuery.Class("Vtiger_Edit_Js",{
	...
	openPopUp : function(e){
		...
		if(jQuery(e.target).closest('.mycdivfield').length) var parentElem = jQuery(e.target).closest('.mycdivfield');
		else  var parentElem = jQuery(e.target).closest('td');
	...
	referenceModulePopupRegisterEvent : function(container){
		...
		container.find('.referenceModulesList').chosen().change(function(e){
			...
			var closestTD = element.closest('.mycdivlabel').next();
	...
	getReferenceSearchParams : function(element){
		...
		if(jQuery(element).closest('.mycdivfield').length) var tdElement = jQuery(element).closest('.mycdivfield');
		else  var tdElement = jQuery(element).closest('td');
		var params = {};
	...
	registerAutoCompleteFields : function(container) {
		...
		container.find('input.autoComplete').autocomplete({
			...
			'select' : function(event, ui ){
				...
				if(element.closest('.mycdivfield').length) var tdElement = element.closest('.mycdivfield');
				else  var tdElement = element.closest('td');
			...
			'change' : function(event, ui) {
				...
				if(element.attr('readonly')== undefined) {
					if(element.closest('.mycdivfield').length) 
						element.closest('.mycdivfield').find('.clearReferenceSelection').trigger('click');
					else  element.closest('td').find('.clearReferenceSelection').trigger('click');
	...
	registerClearReferenceSelectionEvent : function(container) {
		container.find('.clearReferenceSelection').on('click', function(e){
			var element = jQuery(e.currentTarget);
			if(element.closest('.mycdivfield').length)  var parentTdElement = element.closest('.mycdivfield');
			else var parentTdElement = element.closest('td');
	...
	registerReferenceCreate : function(container) {
		...
		container.on('click','.createReferenceRecord', function(e){
			var element = jQuery(e.currentTarget);
			if(element.closest('.mycdivfield').length)  
				var controlElementTd = element.closest('.mycdivfield');
			else var controlElementTd = element.closest('td');
	...
	registerEventForImageDelete : function(){
		...
		formElement.find('.imageDelete').on('click',function(e){
			var element = jQuery(e.currentTarget);
			if(element.closest('.mycdivfield').length) var parentTd = element.closest('.mycdivfield');
			else var parentTd = element.closest('td');
	...
	triggerDisplayTypeEvent : function() {
		...
		if(widthType) {
			if(jQuery('#EditView').find('.mycdivfield').length)
				var elements = jQuery('#EditView').find('.mycdivfield');
			else var elements = jQuery('#EditView').find('td');
