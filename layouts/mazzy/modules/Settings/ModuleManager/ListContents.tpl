		...
		<div class="contents">
			...
			<table class="table table-bordered equalSplit">
						...
						<div class="row-fluid moduleManagerBlock">
							...
							<span class="span1">
								{if $MODULE_MODEL->isExportable()}
									<a href="index.php?module=ModuleManager&parent=Settings&action=ModuleExport&mode=exportModule&forModule={$MODULE_MODEL->get('name')}"><i class="fa fa-download"></i></a>
								{/if}&nbsp;
							</span>
								...
								<span class="span3">
									<span class="btn-group pull-right actions {if !$MODULE_ACTIVE}hide{/if}">
										<button class="btn dropdown-toggle" data-toggle="dropdown">
											<strong>{vtranslate('LBL_SETTINGS', $QUALIFIED_MODULE)}</strong>&nbsp;<i class="fa fa-caret-down"></i>
										</button>
                ...
                <div class="row-fluid" style="padding: 20px 0px;">
                    <a class="btn btn-success" href="{$IMPORT_USER_MODULE_FROM_FILE_URL}">{vtranslate('LBL_INSTALL_FROM_ZIP', $QUALIFIED_MODULE)}</a>
                </div>
	</div>
{/strip}
