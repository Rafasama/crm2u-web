				...
				<td nowrap class="{$WIDTHTYPE}">
								<div class="pull-right actions">
									<span class="actionImages">
										...
										{foreach item=RECORD_LINK from=$LISTVIEW_ENTRY->getRecordLinks() name=listLinkLoop}
											{assign var="RECORD_LINK_URL" value=$RECORD_LINK->getUrl()}
											{assign var="lcounter" value=$smarty.foreach.listLinkLoop.iteration}
											<a class="btn btn-xs    
										{if $lcounter eq ($LISTVIEW_ENTRY->getRecordLinks()|@count)} btn-red-border
										{elseif $lcounter eq (($LISTVIEW_ENTRY->getRecordLinks()|@count)-1)} btn-yellow-border
										{elseif $lcounter eq (($LISTVIEW_ENTRY->getRecordLinks()|@count)-2)} btn-black-border
										{elseif $lcounter eq (($LISTVIEW_ENTRY->getRecordLinks()|@count)-3)} btn-cyan-border
										{elseif $lcounter < (($LISTVIEW_ENTRY->getRecordLinks()|@count)-3)} btn-peas{/if}"
										 {if stripos($RECORD_LINK_URL, 'javascript:')===0} onclick="{$RECORD_LINK_URL|substr:strlen("javascript:")};if(event.stopPropagation){ldelim}event.stopPropagation();{rdelim}else{ldelim}event.cancelBubble=true;{rdelim}" {else} href='{$RECORD_LINK_URL}' {/if}>
