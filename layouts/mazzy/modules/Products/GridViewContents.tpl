...
{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
		...
		<div class="gridViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
			...
			<div class="">
				...
				<div class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' style="position: absolute;
    width: 0;
    height: 0;
    border-top: 45px solid transparent;
    border-right: 45px solid #fff;"></div>
				<div class="myctopgridc" style="text-align: center;">
				{assign var=IMAGE_DETAILS value=$LISTVIEW_ENTRY->getImageDetails()}
					...
					<div class="imageContainer" style="width:100%; margin-top:5px;">
						...
						{if empty($IMAGE_DETAILS)}
							<img src="layouts/mazzy/skins/images/DefaultProductIcon.png" width="300" height="200">
						{else}
							{if !empty($IMAGE_DETAILS[0].path) && !empty({$IMAGE_DETAILS[0].orgname})}
								<img src="{$IMAGE_DETAILS[0].path}_{$IMAGE_DETAILS[0].orgname}" width="300" height="200">
							{else}
								<img src="layouts/mazzy/skins/images/DefaultProductIcon.png" width="300" height="200">
							{/if}
						{/if}
				...
				<a href="{$LISTVIEW_ENTRY->getDetailViewUrl()}">
				<h3> {foreach item=NAME_FIELD from=$NAME_FIELDS}
					...
					<span class="actionImages">
						<a class="btn btn-xs btn-black tooltips" data-toggle="tooltip" data-placement="bottom" title="{vtranslate('LBL_SHOW_FULL_DETAILS')}" href="{$LISTVIEW_ENTRY->getFullDetailViewUrl()}"><i class="fa fa-list"></i></a>
						 {if $MODULE_MODEL->isCommentEnabled()}
							<a class="btn btn-xs btn-primary tooltips" data-toggle="tooltip" data-placement="bottom" title="{vtranslate('ModComments')}" href="{$LISTVIEW_ENTRY->getFullDetailViewUrl()}&mode=showAllComments&tab_label=ModComments"><i class="fa fa-comments"></i></a>
						{/if}
