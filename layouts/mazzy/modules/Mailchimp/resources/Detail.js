...
Vtiger_Detail_Js("Mailchimp_Detail_Js",{},{
	
	loadRelatedList : function(pageNumber){
		var relatedListInstance = new Mailchimp_RelatedList_Js(this.getRecordId(), app.getModuleName(), this.getSelectedTab(), this.getRelatedModuleName());
	...
	registerEventForRelatedListPagination : function(){
		...
		detailContentsHolder.on('click','#relatedListNextPageButton',function(e){
			...
			var relatedController = new Mailchimp_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModuleName);
	...
	registerEventForRelatedList : function(){
		...
		detailContentsHolder.on('click', 'button.massDeleteRelations', function(e){
			var listInstance = Vtiger_List_Js.getInstance();
			var validationResult = listInstance.checkListRecordSelected();
			var url = 'Mailchimp_Detail_Js.triggerMassDeleteRelations("index.php?module=$relatedModuleName&action=MassDelete")';
			if(validationResult != true){
				// get selected ids
				var selectedIds = listInstance.readSelectedIds(true);
				var message = app.vtranslate('LBL_MASS_DELETE_REL_CONFIR');
				Vtiger_Helper_Js.showConfirmationBox({'message' : message}).then(
					function(e) {
						var relatedRecordid;
						var index;
						selectedIds = jQuery.parseJSON(selectedIds);
						var selectedTabElement = thisInstance.getSelectedTab();
						var relatedModuleName = thisInstance.getRelatedModuleName();
						var relatedController = new Mailchimp_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModuleName);
						for (index = 0; index < selectedIds.length; ++index) {
						   relatedRecordid = selectedIds[index];
	...
	registerEventForRelatedTabClick : function(){
		...
		var detailContainer = detailContentsHolder.closest('div.detailViewContainer');
		jQuery('.related', detailContainer).on('click', 'li, a', function(e, urlAttributes){
			...
			thisInstance.loadContents(url,urlAttributes).then(
				function(data){
					...
					var emailEnabledModule = jQuery(data).find('[name="emailEnabledModules"]').val();
					if(emailEnabledModule){
						var listInstance = new Mailchimp_List_Js();
						listInstance.registerEvents();
	...
	registerRelatedListEvents : function(){
		...
		var relatedController = new Mailchimp_RelatedList_Js(this.getRecordId(), app.getModuleName(), selectedTabElement, relatedModuleName);
	...
	registerEvents : function(){
		this.registerRelatedListEvents();
		this._super();
		//Calling registerevents of mailchimp list to handle checkboxs click of related records
		var listInstance = Vtiger_List_Js.getInstance();
		listInstance.registerEvents();
	}
})
