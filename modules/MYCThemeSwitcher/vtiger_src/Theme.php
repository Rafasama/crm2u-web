...
class Vtiger_Theme extends Vtiger_Viewer {
    ...
    public static function getAllSkins(){
    	$util_helper=new Vtiger_Util_Helper();
    	
    	if (class_exists('Vtiger_Util_Helper') && method_exists($util_helper,"getAllSkins")){
	        ...
	        if ($dir = @opendir($themeDir)) {
	            while (($skinName = readdir($dir)) !== false) {
	               ...
	               {
	                   if(is_dir($themeDir."/".$skinName)) {
	                       if(!($skinName[0] == '.'))
	                            $skinsList[$skinName] = $skinName;
	                   }
	               }
	           }
	           closedir($dir);
