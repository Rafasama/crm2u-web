...
class Settings_ExtensionStore_Basic_Action extends Settings_Vtiger_IndexAjax_View {
    ...
    protected function uninstallExtension(Vtiger_Request $request) {
        ...
        if($extnType == 'module'){
            ...
            $response->setResult(array('success' => true, 'message' => 'extension deleted'));
        } else if($extnType == 'layout'){
            $layoutInstance = Settings_ExtensionStore_Extension_Model::getLayoutInstance($extensionName);
            if($layoutInstance) {
                
                $layoutname = $layoutInstance->get('name');
                Vtiger_Layout::deregister($layoutname);
                //remove files
                $layoutDir = "layouts/$layoutname";
                if(isFileAccessible($layoutDir)) {
                    $this->deleteRecursively($layoutDir);
                }
            }
            $response->setResult(array('success' => true, 'message' => 'extension deleted'));
        }   else {
